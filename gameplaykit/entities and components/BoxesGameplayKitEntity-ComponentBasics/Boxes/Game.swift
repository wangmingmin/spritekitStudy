...
class Game: NSObject, SCNSceneRendererDelegate {
    ...
    */
    func setUpEntities() {
        ...
        let redBoxEntity = makeBoxEntityForNodeWithName("redBox")
        
        let yellowBoxEntity = makeBoxEntityForNodeWithName("yellowBox", withParticleComponentNamed: "Fire")
        
        let greenBoxEntity = makeBoxEntityForNodeWithName("greenBox", wantsPlayerControlComponent: true)
        
        let blueBoxEntity = makeBoxEntityForNodeWithName("blueBox", wantsPlayerControlComponent: true, withParticleComponentNamed: "Sparkle")

        // Create the box entity and grab its node from the scene.
        let purpleBoxEntity = GKEntity()
        let purpleBoxNode = scene.rootNode.childNodeWithName("purpleBox", recursively: false)
        
//        let particleEmitter = SCNParticleSystem(named: "Fire", inDirectory: "")!
//        purpleBoxNode?.addParticleSystem(particleEmitter)
    ...
    */
    func addComponentsToComponentSystems() {
        for box in boxEntities {
            particleComponentSystem.addComponentWithEntity(box)
            playerControlComponentSystem.addComponentWithEntity(box)
        }
    }
    ...
    */
    ...
    /**
        ...
        system up to date.
    */
    func renderer(_: SCNSceneRenderer, updateAtTime time: NSTimeInterval) {
        ...
        particleComponentSystem.updateWithDeltaTime(timeSincePreviousUpdate)
    ...
    */
    func makeBoxEntityForNodeWithName(name: String, wantsPlayerControlComponent: Bool = false, withParticleComponentNamed particleComponentName: String? = nil) -> GKEntity {
        // Create the box entity and grab its node from the scene.
        let box = GKEntity()
        guard let boxNode = scene.rootNode.childNodeWithName(name, recursively: false) else {
